...
-- Server-side RPC replacing 4 client-side queries in useGrantEffectiveness.
-- Fixes:
--   1. `.in()` URL overflow with 688+ UUIDs (PostgREST URL limit)
--   2. Mortality always 0% (was filtering exit_date IS NULL then checking exit)
--   3. Single round-trip instead of 4 sequential queries
-- ============================================================================
...
BEGIN
  WITH filtered_animals AS (
    -- Include ALL animals (live + deceased) for proper mortality calculation
    SELECT
      a.id,
  ...
  -- Pre-aggregate records per animal to avoid N+1
  ...
  -- Metrics by acquisition type
  by_type AS (
    SELECT
      fa.acquisition_type,
      COUNT(fa.id) AS animal_count,
      -- Health events per animal (avg across all animals in group)
      ROUND(COALESCE(AVG(COALESCE(hpa.cnt, 0)), 0)::numeric, 1) AS avg_health_events,
      -- Milk per animal (only counting animals that have milk records)
      ROUND(
        CASE
      ...
      -- Mortality rate: deaths / total animals in group * 100
      ROUND(
        CASE
          WHEN COUNT(fa.id) > 0
      ...
      -- Breeding success: confirmed pregnancies / total AI attempts * 100
      ROUND(
        CASE
  ...
  -- Metrics by grant source (only for grant animals)
  by_source AS (
    SELECT
...
-- Grant execute to authenticated users (government dashboard requires auth)
GRANT EXECUTE ON FUNCTION public.get_grant_effectiveness(text, text, text, text)
  TO authenticated;
