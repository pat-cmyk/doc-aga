-- Fix approve_pending_activity to map session values to valid constraint values (AM/PM)
...
BEGIN
  ...
  -- Map various session formats to valid constraint values (AM/PM)
  ...
  _session := CASE 
    ...
    -- Default to AM if time is before noon, PM otherwise
    WHEN _raw_session = '' THEN 
      CASE WHEN EXTRACT(HOUR FROM _record_datetime) < 12 THEN 'AM' ELSE 'PM' END
    ELSE 'AM' -- Fallback to AM for unknown values
  END;
