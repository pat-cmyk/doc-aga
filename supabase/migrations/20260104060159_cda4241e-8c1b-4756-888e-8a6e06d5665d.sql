-- Drop and recreate the get_system_health_metrics function with all fixes
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'approvals', jsonb_build_object(
      ...
      'auto_approved_7d', (SELECT COUNT(*) FROM pending_activities WHERE status = 'approved' AND auto_approve_at IS NOT NULL AND reviewed_at > now() - interval '7 days')
    ),
    ...
    'sync', jsonb_build_object(
      ...
      'avg_duration_ms', 0,
      'failed_24h', (SELECT COUNT(*) FROM sync_queue WHERE created_at > now() - interval '24 hours' AND sync_status = 'failed')
    ),
