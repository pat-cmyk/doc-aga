  ...
  IF NEW.is_sold = true AND (OLD.is_sold IS NULL OR OLD.is_sold = false) THEN
    ...
    IF NOT EXISTS (
      ...
      INSERT INTO farm_revenues (
        farm_id, user_id, amount, source, transaction_date, linked_milk_log_id, notes
      ) VALUES (
        v_farm_id, v_user_id, COALESCE(NEW.sale_amount, 0), 'Milk Sale',
        COALESCE(NEW.record_date, CURRENT_DATE), NEW.id, 'Auto-synced from milk sale'
      );
    END IF;
  END IF;
