-- Drop and recreate get_system_health_metrics with correct stt_analytics column reference
  ...
  SELECT jsonb_build_object(
    ...
    'stt', jsonb_build_object(
      ...
      'avg_latency_ms', COALESCE(
        (SELECT ROUND(AVG(latency_ms)::numeric, 0) FROM stt_analytics WHERE created_at > now() - interval '24 hours'),
        0
      ),
    ...
    'approvals', jsonb_build_object(
      ...
      'auto_approved_7d', (SELECT COUNT(*) FROM pending_activities WHERE status = 'approved' AND auto_approved = true AND reviewed_at > now() - interval '7 days')
    ),
    ...
    'sync', jsonb_build_object(
      'total_syncs_24h', (SELECT COUNT(*) FROM sync_telemetry WHERE created_at > now() - interval '24 hours'),
      'success_rate', COALESCE(
        (SELECT ROUND(
          COUNT(*) FILTER (WHERE status = 'success')::numeric / NULLIF(COUNT(*), 0) * 100, 2
        ) FROM sync_telemetry WHERE created_at > now() - interval '24 hours'),
        100
      ),
      'avg_duration_ms', COALESCE(
        (SELECT ROUND(AVG(duration_ms)::numeric, 0) FROM sync_telemetry WHERE created_at > now() - interval '24 hours'),
        0
      ),
      'failed_24h', (SELECT COUNT(*) FROM sync_telemetry WHERE created_at > now() - interval '24 hours' AND status != 'success')
    ),
