...
-- Bug 1: get_cooperative_health_overview referenced hr.farm_id (doesn't exist) — JOIN through animals
-- Bug 2: get_cooperative_milk_production referenced volume_liters — correct column is liters
...
-- 3. Re-declare get_cooperative_herd_summary (unchanged, for auditability)
  ...
  SELECT json_build_object(
    ...
    'by_species', (
      ...
      FROM (
        ...
        FROM public.animals WHERE farm_id = ANY(_farm_ids) AND is_deleted = false
        GROUP BY livestock_type ORDER BY count DESC
      ) s
    ),
    'by_breed', (
      ...
      FROM (
        ...
        FROM public.animals WHERE farm_id = ANY(_farm_ids) AND is_deleted = false
        GROUP BY breed ORDER BY count DESC LIMIT 20
      ) b
    ),
    'by_gender', (
...
-- 4. Re-declare get_cooperative_financial_summary (unchanged, for auditability)
  ...
  SELECT json_build_object(
    ...
    'revenue_by_farm', (
      ...
      FROM (
        ...
        GROUP BY fr.farm_id, f.name ORDER BY total DESC
      ) r
    )
  ) INTO _result;
